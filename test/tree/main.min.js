	    ...
	    _createClass(Tree, [{
	        ...
	        value: function draw() {
	            ...
	            var angel = this.canvasInfo.angel,
	                ...
	                data = _Calcu2.default.layoutNodeByDagre(this.store.data),
	                nodes = data.nodes,
	                links = data.links;
	            // 清空
	            angel.clear();
	        ...
	        value: function bindClickEvent(nodeShape) {
	            ...
	            nodeShape.on('click', function () {
	                var nodeData = _this3.store.getNodeById(nodeData.id);
	                console.info(originData);
	                console.info(_this3.store.nodeMap[nodeData.id]);
	                //this.draw();
	            });
	        }
	    }]);
...
/***/ function(module, exports) {
	...
	var Calcu = function () {
	    ...
	    _createClass(Calcu, [{
	        ...
	        key: 'dataMixin',
	        value: function dataMixin(originData, nodes, links) {}
	    }]);
	        ...
	        value: function _dataFormat(nodes, links) {
	            ...
	            nodes.forEach(function (node) {
	                if (!_this.nodeMap[node.id]) {
	                    _this.nodeMap[node.id] = Object.assign({}, node);
	                    _this.data.nodes.push(node);
	                }
	            });
	            // 解析连线，绑定节点上下级关系
	            ...
	            links.forEach(function (link) {
	                ...
	                if (!_this.linkMap[key]) {
	                    ...
	                    _this.linkMap[key] = true;
	                    _this.data.links.push(link);
	                }
	            });
	        }
	    }]);
