...
/***/ function(module, exports, __webpack_require__) {
	...
	var TreeDiagram = function () {
	    function TreeDiagram() {
	        ...
	        this.store = new _Store2.default(_.extend({}, opt.data));
	        this.draw({
	            type: 'center',
	    ...
	    _createClass(TreeDiagram, [{
	        ...
	        value: function draw(config) {
	            ...
	            nodes.forEach(function (node) {
	                ...
	                if (node.highLight && node.type !== 'root') {
	                    // 高亮文字背景
	                    var margin = _CONST2.default.highLightLabelMargin,
	                        fontW = textInfo.width,
	                        fontH = 3 * _CONST2.default.fontSize / 4,
	                        rectW = textInfo.width,
	                        rectH = 2 * margin + _CONST2.default.fontSize * 0.75,
	                        circleR = fontH / 2 + margin;

	                    // 包裹高亮边框后，margin值要适当增大，避免与节点重叠
	                    textX = node.type === 'parent' ? textX - circleR : textX + circleR;

	                    var shape = new Angel.Rect({
	                        zlevel: 3,
	                        style: {
	                            brushType: 'both',
	                            fillStyle: _CONST2.default.highLightLabelBackgroud,
	                            strokeStyle: _CONST2.default.highLightLabelBorder,
	                            lineWidth: _CONST2.default.highLightLineWidth,
	                            x: textX,
	                    ...
	                    shape = new Angel.Rect({
	                        ...
	                        style: {
	                            brushType: 'fill',
	                            fillStyle: _CONST2.default.highLightLabelBackgroud,
	                            x: textX - _CONST2.default.highLightLineWidth,
	                            y: node.y + cy - fontH / 2 - margin + _CONST2.default.highLightLineWidth - 1,
	                            w: rectW + 2 * _CONST2.default.highLightLineWidth,
	                            h: rectH - 2 * _CONST2.default.highLightLineWidth + 2
	                        }
	                    });
	                    angel.addShape(shape);
	                }
	                ...
	                var textShape = new Angel.Text({
	                    ...
	                    style: {
	                        ...
	                        x: textX,
	                        y: node.y + cy + 3 * _CONST2.default.fontSize / 8,
	                        text: node.type === 'root' ? '' : textInfo.text
	                    }
	                });
	                angel.addShape(textShape);
	        ...
	        value: function bindHoverEvent(nodeShape) {
	            ...
	            nodeShape.on('mouseover', function (e) {
	                ...
	                showPathTimeout = setTimeout(function () {
	            ...
	            var result = {
	                ...
	                nodes.forEach(function (node) {
	                    ...
	                    if (staticNode && staticNode.id === node.id) {
	                        ...
	                        var sNode = _.extend({ originX: staticNode.x, originY: staticNode.Y }, staticNode),
	                ...
	                nodes.forEach(function (node) {
	                    ...
	                    if (node.children) {
	                        node.children.forEach(function (n) {
	                            result.links.push(_.extend({}, linkMap[node.id + '_' + n.id] || linkMap[n.id + '_' + node.id], {
	                                from: node,
	                                to: n
	                            }));
	                        });
	...
	var Canvas = function () {
	    ...
	    _createClass(Canvas, [{
	        ...
	        value: function bindEvent(canvasDom) {
	            ...
	            // 为canvas加上样式，绑定事件使用
	            var canvasClass = 'canvas-' + new Date().getTime();
	            ...
	            $(document).on('mousedown', '.' + canvasClass, function (e) {
	                isDown = true;
	                startX = e.pageX;
	                startY = e.pageY;
	    ...
	    _createClass(Store, [{
	        ...
	        value: function _dataFormat(nodes, links) {
	            ...
	            nodes.forEach(function (node) {
	                if (!_this.nodeMap[node.id]) {
	                    // 添加默认属性，如颜色、边框厚度等
	                    _this.nodeMap[node.id] = _.extend({
	                        originData: node
	                    }, node);
	                    if (node.type === 'root') {
	        ...
	        value: function getHighLightPath(id) {
	            ...
	            while (node[direction] && node[direction].length > 0) {
	                ...
	                // 沿途线加亮
	                lightLines.push(_.extend({}, linkOpt, { color: linkOpt.color.replace(/[\d\.]+[ ]*\)/, '1)') }));
	                node = parentNode;
	                if (node.type === 'root') {
	                    break;
	                }
