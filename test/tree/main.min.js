...
/***/ function(module, exports, __webpack_require__) {
	...
	var TreeDiagram = function () {
	    ...
	    _createClass(TreeDiagram, [{
	        ...
	        key: 'animation',
	        value: function animation(staticNode) {
	            var _this3 = this;
	            ...
	            var angel = this.canvasInfo.angel,
	                ...
	                data = staticNode ? _Calcu2.default.layoutNodeByStatic(this.store.getExpendData(), staticNode) : _Calcu2.default.layoutNodeByCenter(this.store.getExpendData(), this.store.root),
	                nodes = data.nodes,
	                links = data.links,
	                animationSpend = 200,
	                now = new Date().getTime(),
	                numberCalcu = function numberCalcu(start, end, time) {
	                ...
	                nodes.forEach(function (node) {
	                    // 绘制点
	                    var nodeX = numberCalcu(originPosition[node.id].x, node.x, time) + cx,
	                        //node.x + cx
	                    ...
	                    // 绘制文字
	                    var textInfo = _Calcu2.default.calcuText(node.name, _CONST2.default.fontSize, _CONST2.default.fontMaxLength),
	                        textX = node.type === 'parent' ? nodeX - node.size / 2 - _CONST2.default.fontMargin - textInfo.length : nodeX + node.size / 2 + _CONST2.default.fontMargin;
	                    var textShape = new Angel.Text({
	                        ...
	                        style: {
	                            ...
	                            x: textX,
	                            y: nodeY + _CONST2.default.fontSize / 4,
	                ...
	                links.forEach(function (link) {
	                    var line = new Angel.BezierCurve({
	                        ...
	                        style: {
	                            ...
	                            points: _Calcu2.default.layoutLine({
	        ...
	        key: 'draw',
	        value: function draw(staticNode) {
	            var _this4 = this;
	            ...
	            nodes.forEach(function (node) {
	                ...
	                angel.addShape(circle);
	                // 绑定点击事件
	                _this4.bindEvent(circle);
	                // 绘制文字
	                var textInfo = _Calcu2.default.calcuText(node.name, _CONST2.default.fontSize, _CONST2.default.fontMaxLength),
	                    textX = node.type === 'parent' ? node.x + cx - node.size / 2 - _CONST2.default.fontMargin - textInfo.length : node.x + cx + node.size / 2 + _CONST2.default.fontMargin;
	        ...
	        value: function bindClickEvent(nodeShape) {
	            ...
	            nodeShape.on('click', function () {
	                ...
	                var nodeData = _this5.store.getNodeById(originData.id);
	                ...
	                if (nodeData.isExpend) {
	                    ...
	                    if (!childNodes) {
	                        // 第一次展开操作
	                        if (_this5.opt.onExpand) {
	                            _this5.opt.onExpand.call(_this5, originData, function () {
	                                var nodes = arguments.length <= 0 || arguments[0] === undefined ? [] : arguments[0];
	                                ...
	                                _this5.addData(nodes, links, nodeShape.data);
	                            });
	                        }
	                    } else if (childNodes.length > 0) {
	                        // 展开、重绘
	                        _this5.draw(nodeShape.data);
	                    }
	                }
	                // 收起
	                else {
	        ...
	        value: function bindHoverEvent(nodeShape) {
	            ...
	            var originData = nodeShape.data.originData,
	                ...
	                if (callback) {
	                    callback.call(_this6, e, originData);
	                }
	            };
	            nodeShape.on('mouseover', function (e) {
	                mouseHandle(e, _this6.opt.onNodeMouseEnter);
	            });
	            nodeShape.on('mouseout', function (e) {
	                mouseHandle(e, _this6.opt.onNodeMouseLeave);
	            });
	        }
	    }, {
	        ...
	        value: function bindContextMenuEvent(nodeShape) {
	            ...
	            nodeShape.on('contextmenu', function (e) {
	                if (_this7.opt.onContextMenu) {
	                    _this7.opt.onContextMenu.call(_this7, e, nodeShape.data.originData);
	                }
	            });
	        }
	    }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Calcu = function () {
	    ...
	    _createClass(Calcu, [{
	        ...
	        value: function layoutNodeByCenter(data, centerNode) {
	            // 以centerNode为中心点布局
	            centerNode.originX = centerNode.originY = 0;
	            return this.layoutNodeByStatic(data, centerNode);
	        }
	    }, {
	        key: 'layoutNodeByStatic',
	        value: function layoutNodeByStatic(data, staticNode) {
	            // 节点布局计算
	            ...
	            if (staticNode) {
	                ...
	                var newPosition = g.node(staticNode.id),
	        ...
	        key: 'calcuText',
	        value: function calcuText(text, fontSize, maxLength) {
	            // 计算文字长度(px)，超过最大长度(maxLength)的用...结尾表示
	            text = text || '';
	            text = text.toString();
	            var length = 0.63 * fontSize * text.length;
	            if (text.length > maxLength) {
	                text = text.substring(0, maxLength) + '...';
	                length = 0.63 * fontSize * (text.length - 3) + 0.35 * fontSize * 3;
	            }
	            return {
	                text: text,
	                length: length
	        ...
	        value: function calcuMoveDistance(node, canvas) {
	            ...
	            var isInBound = function isInBound(screenNum) {
	                if (node.x > screenNum * canvas.width || node.x < -screenNum * canvas.width || node.y < -screenNum * canvas.height || node.y > screenNum * canvas.height) {
	                    return false;
	                }
	                return true;
	            },
