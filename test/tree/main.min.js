	    ...
	    _createClass(Tree, [{
	        ...
	        value: function draw() {
	            ...
	            var angel = this.canvasInfo.angel,
	                ...
	                r = 5,
	                // 半径
	            data = _Calcu2.default.layoutNode(this.data);
	                ...
	                if (parentNode) {
	                    var line = new Angel.BezierCurve({
	                        ...
	                        style: {
	                            brushType: 'stroke',
	                            lineWidth: 1,
	                            points: _Calcu2.default.layoutLine(node, parentNode, {
	                                x: cx,
	                                y: cy
	                            })
	                        }
...
/***/ function(module, exports) {
	...
	var Calcu = function () {
	    ...
	    _createClass(Calcu, [{
	        key: "layoutNode_backup",
	        value: function layoutNode_backup(data) {
	            var wd = 100,
	                // 横向间距
	            hd = 15,
	                // 纵向间距
	            ...
	            var iterateNode = function iterateNode(node, parentNode, level) {
	                ...
	                if (node.children) {
	                    ...
	                    if (!firstLeaf) {
	                        ...
	                        var levelNodes = nodesByLevel[level];
	                        if (levelNodes.length === 1) {
	                            // 该节点为level第一个节点
	                            var lastNode = nodesByLevel[level][nodesByLevel[level].length - 2],
	                                lasParentNode = nodesByLevel[level - 1][nodesByLevel[level - 1].length - 2];
	                        } else {
	                            var _lastNode = nodesByLevel[level][nodesByLevel[level].length - 2];
	                            node.x = _lastNode.x;
	                            node.y = _lastNode.y + hd;
	                        }
	                    }
	                }
	                if (parentNode) {
	        ...
	        key: "layoutNode",
	        value: function layoutNode(data) {
	            var wd = 100,
	                // 横向间距
	            hd = 15,
	                // 纵向间距
